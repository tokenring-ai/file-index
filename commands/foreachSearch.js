 ...
 */
  ...
  export const description = "/foreachSearch <search-query> -- <command> - Search for text across files and run a command for each matching file";
  ...
  export async function execute(remainder, registry) {
    ...
    if (parts.length < 2) {
      chatService.errorLine("Missing '--' separator between search query and command");
      this.help(chatService);
      return;
    }
    ...
    if (!fileIndexService) {
      chatService.errorLine("FileIndexService not found. Please add it to your context configuration.");
      return;
    }
    ...
    try {
      ...
      chatService.systemLine(`Searching for: "${query}" and running command: "${command}" on each file...`);
      ...
      chatService.systemLine(`Found ${results.length} result(s). Processing each file...`);

      // Process each result
      for (const result of results) {
        const relativePath = result.path.replace(fileIndexService.baseDirectory, '').replace(/^[/\\]/, '');
