 ...
 */
export default async function ({ query, topK = 10, textWeight = 0.3, fullTextWeight = 0.3, mergeRadius = 1 }, registry) {
 ...
 const maxFullText = Math.max(...Array.from(allHits.values()).map(h => h.fullTextScore));
 ...
 const results = mergedBlocks.map(({ path, indices }) => {
  const blockChunks = indices.map(idx =>
  ...
  return { path: filesystem.relativePath(path), start, end, hybridScore, content };
 });
...
export const description = "Hybrid semantic+full-text+keyword search with merging/deduplication. Returns merged relevant code/text blocks.";
