...
export default async function ({ query, k = 5 }, registry) {
 const chatService = registry.requireFirstServiceByType(ChatService);
  ...
  // Each hit has: {path, chunk_index, content, distance, ...}
   chatService.systemLine(`[FileIndex] Found ${hits.length} matching chunks for query: ${query}\n`);
...
export const description = "Semantic search for file/document code/text chunks using the MariaDB vector database.";
